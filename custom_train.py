...
if __name__ == "__main__":
    ...
    all_dataset = load_dataset(pretrain_data_path, split="train", streaming=True)
    train_dataset = all_dataset.filter(lambda x: get_split(x) < 90)
    eval_dataset = all_dataset.filter(lambda x: 90 <= get_split(x))
